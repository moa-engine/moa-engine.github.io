...
var import_stylify = require("@stylify/stylify");
...
var postCssPrefixer = (0, import_postcss.default)([(0, import_autoprefixer.default)()]);
var hooks = new import_stylify.Hooks();
var defineConfig = (config) => config;
var Bundler = class {
  constructor(config) {
    __publicField(this, "id", (0, import_stylify.createUId)());
  ...
  mergeConfigs(config) {
    ...
    this.compilerConfig = (0, import_stylify.mergeObjects)(this.compilerConfig, config.compiler ?? {});
  ...
  async configure(config, fromConfigFile = null) {
    ...
    if (configOptionType !== "undefined") {
      ...
      config.configFile = configFiles.map((file) => (0, import_normalize_path.default)(file));
    }
    const rawConfig = {
      config,
...
`;
    ...
    const parsedOutputPath = import_path.default.parse(import_path.default.join(this.filesBaseDir ? this.filesBaseDir : "", options.filePath));
    ...
    const variablesFilePath = import_path.default.join(outputDir, outputFile);
  ...
  addBundles(bundles) {
    for (const bundle of bundles) {
      ...
      bundle.outputFile = (0, import_normalize_path.default)(bundle.outputFile);
  ...
  async bundle(bundles = null) {
    ...
    if (this.watchFiles) {
      ...
      for (const [bundleOutputFile, bundleBuildCache] of Object.entries(this.bundlesBuildCache)) {
        if (!import_fs.default.existsSync(bundleOutputFile)) {
          continue;
        }
        ...
        buildsInfo.push({
          name: bundleInfoName,
          size: import_fs.default.statSync(bundleOutputFile).size / 1024,
      ...
      this.log({
        content: `Build done (${((import_perf_hooks.performance.now() - startTime) / 1e3).toFixed(2)} s).`
      });
    }
  ...
  startWatchingConfigFiles() {
    ...
    this.configFilesWatcher = import_chokidar.default.watch(this.configFiles, {
      ignoreInitial: true
    }).on("change", (file) => {
  ...
  processBundle(config) {
    ...
    const bundleRunner = async () => {
      ...
      if (!(bundleConfig.outputFile in this.bundlesBuildCache)) {
        const bundleCompilerConfig = typeof bundleConfig.compiler === "undefined" ? this.compilerConfig : (0, import_stylify.mergeObjects)(this.compilerConfig, bundleConfig.compiler);
        const compiler2 = new import_stylify.Compiler(bundleCompilerConfig);
        const bundleCompilationResult = new import_stylify.CompilationResult({
      ...
      const setupBundleWatcher = (pathsToWatch) => {
        for (const pathToWatch of pathsToWatch) {
          ...
          this.watchedFiles[pathToWatch] = {
            ...
            processing: false,
            watcher: import_chokidar.default.watch(pathToWatch, {
              ignoreInitial: true
            }).on("all", (eventType, fileName) => {
              let isDir;
              try {
                isDir = import_fs.default.lstatSync(pathToWatch).isDirectory();
              } catch (e) {
                return;
              }
              ...
              for (const bundleToProcessIndex of bundlesIndexes) {
                ...
                if (!import_micromatch.default.isMatch(fileName, bundleConfig2.files)) {
                  continue;
                }
      ...
      const outputDir = import_path.default.dirname(bundleConfig.outputFile);
      if (!import_fs.default.existsSync(outputDir)) {
        import_fs.default.mkdirSync(outputDir, { recursive: true });
      }
      ...
      try {
        if (this.autoprefixerEnabled) {
          ...
          import_postcss.default.parse(generatedCss);
        }
      } catch (error) {
        this.log({
          ...
          logFile: `${import_path.default.parse(bundleConfig.outputFile).name}-error.css`
        });
      ...
      bundleBuildCache.buildTime = ((import_perf_hooks.performance.now() - startTime) / 1e3).toFixed(2);
      this.log({
      ...
      if (filesToWatch.length) {
        ...
        for (const fileToWatch of filesToWatch) {
          const parsedPath = import_path.default.parse(fileToWatch);
  ...
  async getFilesToProcess(options) {
    ...
    if (typeof fileMasks !== "undefined") {
      fileMasks = [.../* @__PURE__ */ new Set([
        ...fileMasks,
        ...fileMasks.map((fileMask) => (0, import_normalize_path.default)(fileMask))
      ])];
      filePaths = import_fast_glob.default.sync(fileMasks);
    }
    filePaths = filePaths ?? [];
    ...
    const filesBaseDir = typeof bundleConfig.filesBaseDir === "string" ? (0, import_normalize_path.default)(bundleConfig.filesBaseDir) : null;
    for (const filePath of filePaths) {
      const processFilePath = async (filePath2) => {
        if (!this.checkIfFileExists(filePath2)) {
          return;
        }
        const fileContent = import_fs.default.readFileSync(filePath2).toString();
        ...
        if (filePathsFromContent.length) {
          ...
          filePathsFromContent.join(" ").replace(/\s/g, " ").split(" ").forEach((fileOptionValue) => {
            ...
            let filePathToNormalize = import_path.default.join(import_path.default.dirname(filePath2), fileOptionValue);
            if (fileOptionValue.startsWith(import_path.default.sep)) {
              filePathToNormalize = filesBaseDir ? import_path.default.join(filesBaseDir, fileOptionValue) : fileOptionValue;
            }
            filePathsToProcess.push((0, import_normalize_path.default)(filePathToNormalize));
          });
  ...
  writeFile(filePath, fileContent, trailingNewLine = true) {
    ...
    try {
      const actualFileContent = this.createdFilesContentCache[filePath] ?? import_fs.default.readFileSync(filePath).toString("utf-8");
    ...
    import_fs.default.writeFileSync(filePath, newFileContent);
  }
  ...
  log({
    ...
    if (this.logsDir && ["warn", "error"].includes(severity)) {
      ...
      if (!this.logsDirInitialized && !import_fs.default.existsSync(this.logsDir)) {
        import_fs.default.mkdirSync(this.logsDir);
...
`;
      ...
      import_fs.default.writeFileSync(`${this.logsDir}/${logFile}`, logFileContent, { flag: logWrittingMode });
    }
