...
__export(src_exports, {
  ...
  MinifiedSelectorGenerator: () => MinifiedSelectorGenerator,
  RegExpMatch: () => RegExpMatch,
  Runtime: () => Runtime,
  backslashPlaceholder: () => backslashPlaceholder,
  createUId: () => createUId,
  defaultPreset: () => defaultPreset,
  dollarPlaceholder: () => dollarPlaceholder,
  escapeCssSelector: () => escapeCssSelector,
  getStringOriginalStateAfterReplace: () => getStringOriginalStateAfterReplace,
  hooks: () => hooks,
  ...
  parseHex: () => parseHex,
  parseRgb: () => parseRgb,
  prepareStringForReplace: () => prepareStringForReplace,
  rgbToHex: () => rgbToHex,
  screensSorter: () => screensSorter,
  tokenize: () => tokenize
});
...
var Hooks = class {
  ...
  callHook(hookName, data) {
    ...
    for (const hookCallback of hookListeners[hookName] ?? []) {
      hookCallback(hookData);
    }
    return hookData;
  }
  async callAsyncHook(hookName, data) {
    const hookData = this.createHookData(data);
    const callHook = async (id = 0) => {
      const hook = (hookListeners[hookName] ?? [])[id] ?? null;
      if (!hook) {
        return;
      }
...
var Compiler = class {
  ...
  configure(config) {
    ...
    this.ignoredAreas = [...this.ignoredAreas, ...config.ignoredAreas ?? []].filter((ignoredAreaRegExp, index, self) => {
    ...
    for (const [selector, config] of Object.entries(this.customSelectors)) {
      ...
      for (const customSelector of config.customSelectors) {
        ...
        for ([cssSelector, stylifySelectors] of Object.entries(generatedCssEntries)) {
          ...
          selectorsMap[cssSelector] = [selectorsMap[cssSelector] ?? "", ...stylifySelectors.split(" ")].filter((item, index, self) => {
  ...
  replaceVariableString(string, contentContext) {
    return string.replace(
      ...
      (match, substring) => {
        ...
        return this.cssVariablesEnabled ? `var(--${substring})` : this.variables[substring] ?? match;
      }
    );
  }
...
var CssRecord = class {
  ...
  generateCss(config) {
    if (this.changed || !this.cache) {
      ...
      const cssRecordSelector = config.mangleSelectors ? `${config.mangledSelectorsPrefix ?? ""}${this.mangledSelector}` : this.selector;
      let customSelectors = [];
      let classSelectors = [];
...
var RegExpMatch = class {
  ...
  getCapture(index, defaultValue = void 0) {
    return this.captures[index] ?? defaultValue;
  }
};
...
// packages/stylify/src/Configurator.ts
var import_fs = require("fs");
var import_path = require("path");
var Configurator = class {
  static getExistingConfigFiles(path) {
    const configs = {};
    ["js", "mjs", "cjs"].forEach((suffix) => {
      const filePath = (0, import_path.join)(path, `${this.defaultConfigName}.${suffix}`);
...
__publicField(Configurator, "defaultConfigName", "stylify.config");
